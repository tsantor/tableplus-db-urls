...
package*name := `uv run python -c "import tomllib; n=tomllib.load(open('pyproject.toml','rb'))['project']['name']; print(n.replace('-', '*'))"`wheel_name :=`basename $(ls dist/\*.whl 2>/dev/null | head -n 1) 2>/dev/null || echo ""`
...
find . -name '_.egg-info' -o -name '_.egg' -exec rm -fr {} +

# Remove python file artifacts

[group('cleanup')]
clean-pyc:
find . \( -name '_.pyc' -o -name '_.pyo' -o -name '\*~' -o -name '**pycache**' \) -exec rm -fr {} +
...
[group('misc')]
tree-root:
tree -I '.claude|.tmp|.coverage|htmlcov|dist|build|.eggs|_.egg-info|**pycache**|.pytest_cache|.ruff_cache|.tox|.vscode|node_modules|_.csv'
...
rsync -avz . {{user}}@{{host}}:{{remote_dir}} --delete \
 ...
 --exclude=".coverage" --exclude='htmlcov' --exclude='**pycache**' \
